...
export default class MainCharts extends Vue {
  ...
  convertData = function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
      var dataItem = data[i];
      var fromCoord = [dataItem.originlng, dataItem.originlat];
      var toCoord = [dataItem.destlng, dataItem.destlat];
      if (fromCoord && toCoord) {
        res.push({
          fromName: dataItem.originCity,
          toName: dataItem.destinationCity,
  ...
  destination = array => {
    ...
    for (let index = 0; index < array.length; index++) {
      ...
      a.push({
        name: element.destinationCity,
        value: [element.destlng, element.destlat, 20]
      });
    }
  ...
  drawLineFromList(list) {
    ...
    this.series[0].data = this.convertData(this.MAPData.slice(0, 100));

    this.series[1].data = this.convertData(this.MAPData.slice(0, 100));
    // this.series[2].data = this.destination(this.MAPData.slice(0, 200));

    this.series[2].data = this.MAPData.slice(0, 100).map(item => ({
      name: item.destinationCity,
      value: [item.destlng, item.destlat, 20]
    }));
  ...
  async getMonthTradeNum() {
    ...
    this.raiseToNum("month", Math.floor(res.data[0].sumMonthFeeAmount));
  }
  /**
   * 获取季度交易额
   */
  async getQuarterTradeNum() {
    ...
    this.raiseToNum("quater", Math.floor(res.data[0].sumQuaterFeeAmount));
  }
  ...
  async getYearTradeNum() {
    ...
    this.raiseToNum("year", Math.floor(res.data[0].sumYearFeeAmount));
  }
      ...
      if (this.total_trade_num[i] != tempList[i]) {
        $(".trade-num-box-" + i.toString()).animate(
          ...
          () => {
            ...
            $(".trade-num-box-" + i.toString()).animate({ top: "2rem" }, 250);
          }
        );
      }
    }
