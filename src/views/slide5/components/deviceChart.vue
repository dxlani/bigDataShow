<template>
    <div id="deviceChart" style="height: 100%;width:100%">
    </div>
</template>
...
export default class deviceChart extends Vue {
  ...
  created(){
    ...
    this.myChartOption.series[0].data.push(random(8500,11500));//给Y轴 插入数据
    }
    
  }
  mounted() {
    this.drawChart();
    var deviceTimer=setInterval(()=>{
        var time = new Date();
        // console.log('aaaa',new Date(),new Date().getSeconds());
        ...
        this.deviceChart.setOption(this.myChartOption);
        }
      }, 1000);
  }
